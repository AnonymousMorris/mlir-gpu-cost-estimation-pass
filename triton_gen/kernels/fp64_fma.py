...
def fp64_fma_kernel(
    ...
    # Consumer GPUs have far less FP64 than memory throughput. Independent
    # chains expose FP64 instruction-level parallelism without extra traffic.
    acc0 = x
    acc1 = x + 0.25
    acc2 = x - 0.25
    acc3 = x * 0.5
...
KERNEL = fp64_fma_kernel


COMPUTE_ITERATIONS = 32
BENCHMARK_SIZES = (2**16, 2**18, 2**20)


def init_args(device):
    return make_args(device, size=2**16, iterations=COMPUTE_ITERATIONS, block_size=256)
...
def make_args(device, size, iterations, block_size):
    ...
    factor = torch.empty(size, device=device, dtype=torch.float64).uniform_(0.999, 1.001)
